...
```text
脅威モデルの整理 (jti 防御が効く範囲は狭い):

  ケース A: runner の完全侵害
    走行中の runner 上で OIDC JWT を読める攻撃者は、同じプロセスのメモリ/
    ディスクにある ephemeral 秘密鍵と発行済み証明書も読める。この場合は
    replay する必要すらなく、すでに「動く cert + key」を握っている。
    → jti による replay 防止を足しても守れるものがない。

  ケース B: トークン単独漏洩 (秘密鍵は漏れない)
    JWT は bearer token なので、秘密鍵が外に出ない経路でも漏れ得る:
      - CI ログ / 前段プロキシ / WAF のリクエストログ / APM trace に紛れる
        (応答の証明書は残らない)
      - SSRF や悪意ある third-party action に「CA audience の token だけ」を
        吐かせて exfil する (秘密鍵は渡らない)
      - CA 側でリクエストボディだけが収集される構成
    この場合、攻撃者は JWT の exp まで自分の公開鍵で証明書を何枚でも発行できる。
    → jti 防御の取り分はこのケース B に限定される。
...
残るリスク:
  ケース B の時間窓の中では、正規 runner と並行して攻撃者も証明書を取得できる。
```
...
jti ベースの一回限り消費 (replay 防止):
  ...
  ただし上記のとおり、守れるのはケース B (トークン単独漏洩) だけで、
  ケース A (runner 完全侵害) には無力。費用対効果はこの前提で評価する。
状態の持ち方が運用形態で変わる:
  ...
  - jti を持たない / 一意でない provider もあり得るため、policy で有効・無効を
    切り替えられる設計が前提。
レース / DoS の副作用:
  正規 run より先に攻撃者が同じ jti を消費すると、攻撃者が証明書を得たうえに
  正規 run を deny できてしまう (一意消費が DoS になり得る)。導入するなら
  「失敗した発行では jti を消費しない」等の扱いを併せて設計する必要がある。
```

当面の対応方針: MVP では実装しない。守れる範囲がケース B に限定されるのに対し、
共有ステート・可用性・レース処理のコストが大きいため。代わりに README /
Security model に「OIDC token は exp 内で replay 可能 (ただし秘密鍵が漏れない
経路に限る)」旨を明記する。あわせて、replay 調査を可能にするため監査ログに
`jti` (取得できる場合) を含めることを検討する (section 14)。
