...
- **CA key source is exactly one of** `--ca-key-file` / `OIDC_SSH_CA_KEY_FILE` / `OIDC_SSH_CA_KEY`; zero or multiple sources → startup failure. Key file permissions looser than 0600 → startup failure. ed25519 only in MVP; only `ssh-ed25519` client public keys accepted (certificate keys rejected).
